    ...
    while (1)
        {
            ...
            /* Compute 3D Shock Vector Magnitude */
            ...
            /* Convert float to integer parts for lightweight PRINTF */
            int32_t gInt = (int32_t)totalG;
            int32_t gDec = (int32_t)((totalG - (double)gInt) * 100);
            if (gDec < 0) gDec *= -1;

            /* Print using standard integer specifiers */
            PRINTF("X:%5d | Y:%5d | Z:%5d | Magnitude: %d.%02dg\r\n", xRaw, yRaw, zRaw, gInt, gDec);
        }
}
